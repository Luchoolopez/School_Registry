...
import type { CreateSchoolDTO } from '../../types/school.types';
...
export const CreateSchoolModal: React.FC<Props> = ({ isOpen, onClose, onCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Error al crear la escuela');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md p-6 border border-gray-100 dark:border-gray-800">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Crear Escuela</h3>
